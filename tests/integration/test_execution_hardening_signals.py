...
def test_asset_universe_supported_quotes_execution_hardening():
    """v7+: Universe supports USDC, USDT, and FDUSD quotes. BUSD deprecated."""
    from execution.utils.metrics import is_in_asset_universe

    assert is_in_asset_universe("BTCUSDC")
    assert is_in_asset_universe("BTCUSDT")  # Now supported
    assert is_in_asset_universe("ETHFDUSD")  # Now supported
    assert not is_in_asset_universe("ETHBUSD")  # BUSD deprecated
