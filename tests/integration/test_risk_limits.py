...
def test_check_order_returns_detail_on_nav_warning(monkeypatch, mock_clean_drawdown_state):
    st = RiskState()
    cfg = _base_cfg()
    ...
    # Also mock nav history to prevent circuit breaker interference
    monkeypatch.setattr(
        risk_limits_module,
...
def test_trade_caps_allow_moderate_notional(monkeypatch, mock_clean_drawdown_state):
...
def test_guardrail_symbol_leverage_override_takes_precedence(monkeypatch, mock_clean_drawdown_state):
    # Mock nav history to prevent circuit breaker interference
    monkeypatch.setattr(risk_limits_module, "_nav_history_from_log", lambda limit=200: [])
