...
def test_nav_fresh_ok(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that fresh NAV snapshots pass the freshness check.
    
    Note: This test requires proper working directory isolation.
    The NAV paths are relative and expect logs/cache/ or cache/ directories.
    """
    _reload_with_tmpdir(tmp_path, monkeypatch)
    # Create snapshot at expected path (logs/cache/ is preferred)
    _write_snapshot(tmp_path / "logs" / "cache" / "nav_confirmed.json", age_seconds=5, sources_ok=True)
