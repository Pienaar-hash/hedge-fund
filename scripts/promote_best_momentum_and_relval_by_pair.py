...
def score_curve(path):
    try:
        ...
        if len(df) < 20 or df["equity"].nunique() <= 1:
            return None
...
def extract_group_key(fname, strategy):
    ...
    if strategy == "momentum":
        try:
            return parts[2].lower()  # e.g., btcusdt
        except:
        ...
        try:
            base = parts[2].lower()
            quote = parts[3].lower()
            if base != "relative" and quote != "value":
                return f"{base}_{quote}"
        except:
            pass
        return "relval_unknown"
    return "misc"
...
for fname in os.listdir(LOG_DIR):
    ...
    if result is None:
        print(f"⚠️ Skipping invalid or low-quality: {fname}")
        continue
