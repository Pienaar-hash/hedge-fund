...
def get(path, **params):
    r=requests.get(BASE+path, params=params, headers={"X-MBX-APIKEY":AK}, timeout=10); r.raise_for_status(); return r.json()
def post(path, params):
    q="&".join(f"{k}={v}" for k,v in params.items())
    r=requests.post(BASE+path+"?"+q+"&signature="+sig(q), headers={"X-MBX-APIKEY":AK}, timeout=10)
    print("POST", path, r.status_code, r.text); r.raise_for_status(); return r.json()

def round_step(q, step): return math.floor(q/step)*step

def place_from_intent(intent):
    sym = intent["symbol"]
    px  = float(get("/fapi/v1/ticker/price", symbol=sym)["price"])
    info= get("/fapi/v1/exchangeInfo")
    fx  = {x["filterType"]: x for x in next(s for s in info["symbols"] if s["symbol"]==sym)["filters"]}
    step= float(fx["LOT_SIZE"]["stepSize"]); min_q=float(fx["LOT_SIZE"]["minQty"])
    cap = float(intent.get("capital_per_trade", 0.0)); lev=float(intent.get("leverage",1.0))
    notional = max(1.0, cap*lev)
    qty = round_step(notional/px, step)
    if qty < min_q: qty = min_q
    ...
    params = {"symbol": sym, "side": side, "type": "MARKET", "quantity": qty,
              "timestamp": ts, "positionSide": posSide}
