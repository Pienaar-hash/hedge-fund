...
- Risk/telemetry: veto logs (`logs/execution/risk_vetoes.jsonl`), router metrics (`logs/state/router_health.json`), intel scores (kpis/expectancy/symbol_scores).
...
### Reading contract
- Expects `logs/state/nav.json`, `positions.json`, `router_health.json`, `pipeline_v6_shadow_head.json`/`pipeline_v6_compare_summary.json`, and the intel KPI files (`kpis_v7.json`, `expectancy_v6.json`, `symbol_scores_v6.json`); tolerates missing files with fallbacks.
