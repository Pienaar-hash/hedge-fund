...
def render_performance_block(
    ...
    # Format helper — every card shows a real dollar value
    ...
    # Sub-labels: show "no trades" when window PnL is exactly 0
    d_sub = "<br><span style='font-size:9px;color:#555'>no trades</span>" if daily_pnl == 0 else ""
    w_sub = "<br><span style='font-size:9px;color:#555'>no trades</span>" if weekly_pnl == 0 else ""
    m_sub = "<br><span style='font-size:9px;color:#555'>no trades</span>" if monthly_pnl == 0 else ""
    ...
    <div class="quant-card">
        <div class="performance-grid">
            ...
            <div class="perf-card">
                <div class="perf-label">7d PnL{w_sub}</div>
                <div class="perf-value {w_cls}">{w_val}</div>
            </div>
            <div class="perf-card">
                <div class="perf-label">30d PnL{m_sub}</div>
