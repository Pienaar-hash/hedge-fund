...
def build_kpi_cards(
    ...
    # All-time PnL: always use episode ledger (covers full trading history).
    # NAV-delta only covers the nav_log window (may start mid-history after
    # log resets) and would silently understate losses incurred before the
    # log anchor.  NAV-delta is still used for windowed metrics (24h/7d/30d)
    # where its span authority is appropriate.
    _at_pnl = 0.0
    if episode_ledger:
