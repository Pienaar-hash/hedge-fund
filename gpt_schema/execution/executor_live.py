    ...
    try:
        ...
        if not reduce_only:
            ...
            max_symbol_cap = _merge_caps(
                sizing_cfg.get("max_symbol_exposure_pct"),
                risk_global_caps.get("max_symbol_exposure_pct"),
            )
            max_portfolio_cap = _merge_caps(
                sizing_cfg.get("max_gross_exposure_pct"),
            ...
            max_trade_nav_cap = _merge_caps(
                sizing_cfg.get("max_trade_nav_pct"),
                risk_global_caps.get("max_trade_nav_pct"),
            )
            trade_equity_cap = _merge_caps(
                sizing_cfg.get("trade_equity_nav_pct"),
                risk_global_caps.get("trade_equity_nav_pct"),
            )
            sizing_per_symbol_leverage = sizing_cfg.get("per_symbol_leverage") or {}
            per_symbol_leverage: Dict[str, float] = {}
            if isinstance(sizing_per_symbol_leverage, Mapping):
                for key, value in sizing_per_symbol_leverage.items():
                    try:
                        per_symbol_leverage[str(key).upper()] = float(value)
                    except Exception:
                        continue
            ...
            symbols_for_leverage = set(per_symbol_leverage.keys()) | set(risk_per_symbol.keys())
            symbols_for_leverage.add(str(symbol).upper())
            for sym_key in symbols_for_leverage:
                current_val = per_symbol_leverage.get(sym_key, default_leverage)
                risk_sym_lev = 0.0
